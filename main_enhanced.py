...
def scrape_products_enhanced(product_links: Optional[Dict[str, List[str]]] = None) -> List[Dict[str, Any]]:
    ...
    logger.info(
        f"Scraping completed in {end_time - start_time:.2f}s. "
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt < max_retries - 1:
                            logger.warning(
                                f"Email attempt {attempt + 1} failed to {recipient}: {e}. Retrying in {retry_delay}s..."
                            )
...
def run_enhanced_scheduler():
    ...
    logger.info(
        f"Configuration: {len(config.PRODUCT_LINKS)} retailers, "
