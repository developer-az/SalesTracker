...
def api_scrape():
    ...
    try:
        ...
        return jsonify({"success": True, "products": results, "timestamp": datetime.now().isoformat()})
...
def api_send_test_email():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Test email sent successfully", "timestamp": app_state.last_email_sent})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_start_scheduler():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Scheduler started", "is_running": app_state.is_running})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_stop_scheduler():
    ...
    try:
        app_state.is_running = False
        return jsonify({"success": True, "message": "Scheduler stop requested", "is_running": app_state.is_running})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_recipients():
    try:
        if request.method == "GET":
            return jsonify({"success": True, "recipients": recipients_store.load_recipients()})
