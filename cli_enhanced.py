...
def setup_logging(verbose=False, quiet=False):
    ...
    logging.basicConfig(level=level, format="%(asctime)s - %(levelname)s - %(message)s", handlers=[logging.StreamHandler()])
...
Examples:
  ...
  %(prog)s run                              # Start scheduler
    ...
    if args.command == "test-scraping":
        test_enhanced_scraping(retailer_filter=args.retailer, use_cache=not args.no_cache)
